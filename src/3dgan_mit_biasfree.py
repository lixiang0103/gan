...
def trainGAN(is_dummy=False, checkpoint=None):
    ...
    d_output_x = tf.maximum(tf.minimum(d_output_x, 0.99), 0.01)    # d_output_x is between 0.01 and 0.99
    ...
    with tf.Session() as sess:  
        ...
        if is_dummy:
            ...
            voxList = d.getAll(obj=obj, train=True, is_local=is_local, obj_ratio=obj_ratio)
            print 'Using ' + obj + ' Data'
        # volumes *= 2.0
        # volumes -= 1.0
        for epoch in range(n_epochs):
            for batch_count in range(len(voxList)/batch_size):
                volumes = np.zeros([batch_size,cube_len,cube_len,cube_len],dtype = np.bool)
                # each batch get two npy
                for index in range(batch_count*batch_size,(batch_count+1)*batch_size):
                ...
                discriminator_loss = sess.run(d_loss,feed_dict={z_vector: z, x_vector: x})
