...
SERVER_PATH = '/home/ets/lixiang/tf-3dgan-master/sample-data/volumetric_data/'
...
def getVFByMarchingCubes(voxels, threshold=0.5):
    v, f,normals,values=sk.marching_cubes(voxels, level=threshold)
    return v, f
...
def getAll(obj='airplane',train=True, is_local=False, cube_len=64, obj_ratio=1.0):
    ...
    objPath = LOCAL_PATH + obj + "/30" + "/train/"
    pre_voxList = []
   # c =  int(obj_ratio*len(fileList))
    for f in os.listdir(objPath):
        if f.endswith('.npy'):
            pre_voxList.append(os.path.join(objPath, f))
    index = int(obj_ratio * len(pre_voxList))
    voxList = pre_voxList[0:index]
    return voxList
